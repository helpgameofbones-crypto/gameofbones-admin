      ...
      <div className='p-6 max-w-7xl mx-auto'>
        ...
        <div className='grid grid-cols-2 gap-4 mb-6'>
          <div className='bg-white rounded-2xl p-5 shadow-sm border border-gray-100'>
            ...
            <div className='text-3xl font-bold text-gray-900'>Rs {loading ? '...' : weekRevenue.toLocaleString('en-IN')}</div>
          </div>
          <div className='bg-white rounded-2xl p-5 shadow-sm border border-gray-100'>
            <div className='text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1'>Last 30 Days Revenue</div>
            <div className='text-3xl font-bold text-gray-900'>Rs {loading ? '...' : monthRevenue.toLocaleString('en-IN')}</div>
          </div>
        </div>
              ...
              {loading ? (
                ...
                return (
                  <tr key={order.id} className='hover:bg-gray-50 transition-colors'>
                    ...
                    <td className='px-4 py-3 font-bold text-gray-900'>Rs {(order.grand_total || order.total_amount || 0).toLocaleString('en-IN')}</td>
                    <td className='px-4 py-3'>
                      <span className='text-xs px-2 py-1 rounded-full font-medium' style={{ background: order.payment_method === 'cod' ? '#fef3c7' : '#dcfce7', color: order.payment_method === 'cod' ? '#92400e' : '#166534' }}>
                        {order.payment_method?.toUpperCase()}
                      </span>
                    </td>
                    <td className='px-4 py-3'>
                      <span className='text-xs px-2 py-1 rounded-full font-medium capitalize' style={{ background: sc.bg, color: sc.color }}>
                        {order.status?.replace(/_/g, ' ')}
                      </span>
                    </td>
                    <td className='px-4 py-3 text-xs text-gray-400'>{new Date(order.created_at).toLocaleDateString('en-IN')}</td>
                  </tr>
                )
              })}
            </tbody>

...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!dogId) {
      return NextResponse.json({ error: 'Missing dogId' }, { status: 400, headers });
    }
    ...
    if (fetchError || !birthday) {
      return NextResponse.json({ error: 'Dog not found' }, { status: 404, headers });
    }

    if (!birthday.email) {
      return NextResponse.json({ error: 'No email on file' }, { status: 400, headers });
    }
    ...
    console.log(`Birthday email sent to ${birthday.email} for ${birthday.dog_name}`);

    return NextResponse.json({ success: true, message: 'Email sent successfully' }, { headers });
  } catch (error: any) {
    console.error('Birthday email error:', error);
    return NextResponse.json({ error: error.message || 'Failed to send email' }, { status: 500, headers: corsHeaders(req) });
  }
}

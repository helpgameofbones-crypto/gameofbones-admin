...
export default function OrdersPage() {
  ...
  async function updateStatus(orderId: string, newStatus: string) {
    ...
    // Send dispatch or delivery email
    if (newStatus === 'dispatched' || newStatus === 'delivered') {
      ...
      if (orderData && orderData.customer_email) {
        await fetch('/api/orders', {
          ...
          body: JSON.stringify({
            type: newStatus === 'dispatched' ? 'dispatch' : 'delivery',
            ref: orderData.ref,
            customer_name: orderData.customer_name,
            customer_email: orderData.customer_email,
            awb: orderData.delhivery_awb,
          })
        })
      }
    }
  ...
  function printPackingSlip(order: any) {
    ...
    const html = `<!DOCTYPE html><html><head><title>Packing Slip ${order.ref}</title>
        ...
        <div style="margin-top:12px;padding-top:8px;border-top:1px solid #e5e7eb;font-size:12px">
          <div style="display:flex;justify-content:space-between">
            ...
            <span style="font-weight:bold;color:${order.payment_method === 'cod' ? '#f59e0b' : '#10b981'}">
              ${order.payment_method === 'cod' ? 'COD — Rs ' + order.grand_total : 'PREPAID'}
            </span>
          </div>
          ...
          ${order.delhivery_awb ? `<div style="display:flex;justify-content:space-between;margin-top:4px">
            <span>AWB:</span><span style="font-weight:bold;font-family:monospace">${order.delhivery_awb}</span>
          </div>` : ''}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen" style={{ background: '#f9f6f2' }}>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {/* Pincode checker */}
        <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-4 flex items-center gap-3 flex-wrap">
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50 transition-colors"
                    ...
                    <td className="px-4 py-3 font-mono font-bold text-sm" style={{ color: '#c8973a' }}>
                      {order.ref}
                      {order.notes && (
                        <div className="text-xs font-normal italic" style={{ color: '#9ca3af' }}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex justify-between text-sm pt-2">
                  <span style={{ color: '#374151' }}>Total Weight</span>
                  <span className="font-bold">{getWeight(selected.items || [])}g</span>
                </div>
              ...
              {selected.delhivery_awb && (
                <div>
                  <div className="text-xs font-semibold uppercase mb-2" style={{ color: '#6b7280' }}>AWB Number</div>
              ...
              <div>
                <div className="text-xs font-semibold uppercase mb-2" style={{ color: '#6b7280' }}>Internal Note</div>
                <textarea
                  value={noteText}
              ...
              <div>
                <div className="text-xs font-semibold uppercase mb-2" style={{ color: '#6b7280' }}>RTO Risk</div>
                {(() => {

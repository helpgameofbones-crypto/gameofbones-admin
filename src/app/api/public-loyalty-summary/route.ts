...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [ordersResult, referralsResult, rewardsResult, customerResult] = await Promise.all([
      supabase.from('orders').select('grand_total,total_amount,customer_phone,pii_phone_ciphertext,pii_phone_hash').limit(5000),
    ...
    const spendPoints = Math.floor(orders.reduce((sum, order) => sum + Number(order.grand_total || order.total_amount || 0), 0) / 10)
    ...
    return NextResponse.json({ points: Math.max(spendPoints + referralPoints + reviewPoints, profilePoints) }, { headers })

...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!EMAIL_RE.test(email) || !SOURCE_RE.test(source) || (phone && !/^\+?\d{10,13}$/.test(phone)) || !marketingConsent) return NextResponse.json({ error: 'Please provide valid details and marketing consent.' }, { status: 400, headers })
    const { error } = await supabase.from('email_captures').insert({ email, name: name || null, phone: phone || null, source, status: 'active', prize: cleanText(body.prize, 100) || null, coupon_code: cleanText(body.coupon_code, 80) || null, marketing_consent: true, marketing_consent_at: new Date().toISOString(), privacy_notice_version: cleanText(body.privacy_notice_version, 40) || null, pii_email_ciphertext: encryptPii(email), pii_name_ciphertext: encryptPii(name), pii_phone_ciphertext: encryptPii(phone), pii_email_hash: piiHash(normalizeEmailForHash(email)), pii_phone_hash: piiHash(normalizePhoneForHash(phone)), pii_key_version: 1 })
